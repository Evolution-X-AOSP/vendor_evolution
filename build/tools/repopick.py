...
if __name__ == '__main__':
    ...
    parser.add_argument('-ap',
                        '--ap',
                        action='store_true',
                        help='use apply-patch method')
    parser.add_argument('-P',
                        '--path',
                        help='use the specified path for the change')
    parser.add_argument('-t',
                        '--topic',
                        help='pick all commits from a specified topic')
    parser.add_argument('-Q',
                        '--query',
    ...
    for change in change_numbers:
        ...
        mergables.append({
            ...
            None,
            'patchset':
            review['revisions'][review['current_revision']]['_number'],
        })
        ...
        if patchset:
            try:
                ...
                args.quiet or print(
                    'ERROR: The patch set {0}/{1} could not be found, using CURRENT_REVISION instead.'
    ...
    for item in mergables:
        ...
        elif item['project'] in project_name_to_data and len(
                project_name_to_data[item['project']]) == 1:
            local_branch = list(project_name_to_data[item['project']])[0]
            project_path = project_name_to_data[item['project']][local_branch]
            print(
                'WARNING: Project {0} has a different branch ("{1}" != "{2}")'.
                format(project_path, local_branch, item['branch']))
        ...
        for i in range(0, check_picked_count):
            ...
            if 'Change-Id:' in output:
                ...
                if head_change_id.strip() == item['change_id']:
                    print('Skipping {0} - already picked in {1} as HEAD~{2}'.
        ...
        if not args.quiet:
            ...
            print('--> Change number: {0} (Patch Set {1})'.format(
        ...
        if args.ap:
            patch_url = '{0}/{1}/commit/{2}.patch'.format(
                default_github, item['project'], item['current_revision'])
